...
<div
	class="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-indigo-400/60 hover:shadow-md dark:border-white/10 dark:bg-slate-900/50 dark:shadow-none dark:hover:bg-slate-900/80"
>
	<!-- Prayer content -->
	<div class="mb-4">
			...
			<div class="mb-3 flex items-center space-x-2">
				<img
					src={ownerProfile?.photoURL ??
						`https://ui-avatars.com/api/?name=${ownerProfile?.displayName ?? 'User'}`}
					alt="Owner Profile"
		...
		<a href="/prayers/{prayer.id}" class="cursor-pointer">
			{#if prayer.summary}
				...
				<h4
					class="mb-2 text-xl font-semibold text-slate-900 transition-colors hover:text-indigo-600 dark:text-white dark:hover:text-indigo-300"
				>
					{prayer.summary}
				</h4>
				{#if prayer.description}
					<p
						class="whitespace-pre-wrap leading-relaxed text-slate-600 {showFullDescription
							? ''
							: 'line-clamp-1'} dark:text-slate-300"
					>
						{prayer.description}
					</p>
				{/if}
			{:else if (prayer as any).content}
				...
				<p
					class="line-clamp-1 whitespace-pre-wrap text-lg leading-relaxed text-slate-700 dark:text-slate-200"
				>
					{(prayer as any).content}
				</p>
			{:else}
				<!-- No content at all -->
				<p class="italic text-slate-400 dark:text-slate-500">No content available</p>
			{/if}
		</a>
		...
		{#if latestUpdate && showLatestUpdate}
			<div
				class="mt-3 rounded-lg border border-slate-900/10 bg-slate-100/80 p-3 dark:border-slate-700/50 dark:bg-slate-800/30"
			>
	...
	<div
		class="flex items-center justify-between border-t border-slate-200 pt-4 dark:border-white/10"
	>
