...
export const notifyGroupMembersPrayerShared = async (
	prayerId: string,
	prayerSummary: string,
	sharedWith: string[]
) => {
		...
		const groupPromises = sharedWith.map(async (groupId) => {
			...
			if (groupDoc.exists()) {
				...
				const notifications = members
					...
					.map((memberId: string) =>
						sendNotification(memberId, 'prayer_shared', prayerId, prayerSummary, groupId, groupName)
					);
...
export const notifyGroupMembersPrayerAnswered = async (
	prayerId: string,
	prayerSummary: string,
	sharedWith: string[]
) => {
