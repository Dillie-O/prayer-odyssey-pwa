...
import {
	sendNotification,
	notifyGroupMembersPrayerAnswered,
	notifyGroupMembersPrayerShared
} from './notifications';
	...
	if (u) {
		...
		if (userGroupIds.length > 0) {
			q = query(
				...
				or(where('ownerId', '==', u.uid), where('sharedWith', 'array-contains-any', userGroupIds)),
...
export const subscribeToPrayerUpdates = (prayerId: string) => {
	const q = query(collection(db, 'prayers', prayerId, 'updates'), orderBy('createdAt', 'desc'));
