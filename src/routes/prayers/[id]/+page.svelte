...
{:else}
	<div class="mx-auto max-w-4xl space-y-6">
		...
		<div class="flex items-center gap-3">
			<a
				href="/"
				class="-ml-2 rounded-lg p-2 text-gray-500 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-white"
		...
		>
			<div class="mb-4">
				...
				{#if prayer && !isOwner}
					<div
						class="mb-4 flex items-center space-x-2 border-b border-slate-900/5 pb-4 dark:border-white/5"
					>
						<img
							src={ownerProfile?.photoURL ||
								(ownerProfile?.displayName
									? `https://ui-avatars.com/api/?name=${ownerProfile.displayName}`
				...
				{#if prayer.summary}
					<h2 class="mb-3 text-2xl font-bold text-white">{prayer.summary}</h2>
					{#if prayer.description}
						<p
							class="whitespace-pre-wrap text-lg leading-relaxed text-slate-600 dark:text-slate-300"
						>
							{prayer.description}
						</p>
					{/if}
				{:else if (prayer as any).content}
					<p class="whitespace-pre-wrap text-xl leading-relaxed text-slate-700 dark:text-slate-200">
				...
				{#if isOwner && sharedGroupNames.length > 0}
					<div class="mt-6 flex flex-wrap gap-2">
						<span
							class="mb-1 w-full text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500"
							>Shared with:</span
						>
						{#each sharedGroupNames as groupName}
							<span
								class="inline-flex items-center rounded-full border border-slate-900/10 bg-slate-200 px-2.5 py-0.5 text-xs font-medium text-slate-600 dark:border-white/5 dark:bg-slate-800 dark:text-slate-300"
							>
								{groupName}
							</span>
						{/each}
			...
			<div
				class="flex flex-wrap items-center justify-between gap-3 border-t border-slate-900/10 pt-4 dark:border-white/10"
			>
					...
					<div class="flex items-center space-x-2">
						...
						>
							...
							{:else}
								...
								>
									<path
										d="M12 14.5c.667 0 1.333-.333 2-1 1.333-1.333 1.333-3.333 0-4.667l-2-2.333-2 2.333c-1.333 1.333-1.333 3.333 0 4.667.667.667 1.333 1 2 1Z"
									/>
									<path
										d="M17.5 19c3-2 4.5-4.5 4.5-7.5H2c0 3 1.5 5.5 4.5 7.5.667 0 1.333-.333 2-1a4 4 0 0 0 6 0c.667.667 1.333 1 2 1"
									/>
								</svg>
							{/if}
						</button>
						...
						<button
							...
							class="rounded-full p-3 text-indigo-400 transition-colors hover:bg-indigo-400/10 hover:text-indigo-300"
						...
						<button
							...
							class="rounded-full p-3 text-blue-400 transition-colors hover:bg-blue-400/10 hover:text-blue-300"
							title="Edit Prayer"
						>
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path
						...
						<button
							...
							class="rounded-full p-3 text-rose-400 transition-colors hover:bg-rose-400/10 hover:text-rose-300"
							title="Delete"
						>
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path
				...
				{:else}
					<!-- Spacer for non-owners to maintain alignment -->
					<div></div>
				{/if}

				<!-- Date and Status (right justified) -->
		...
		>
			<div class="mb-4 flex items-center justify-between">
				...
				{#if isOwner}
					<button
						...
						class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-500"
					>
						+ Add Update
					</button>
				{/if}
			...
			<div class="space-y-3">
				{#each updates as update (update.id)}
					<div
						class="rounded-lg border border-slate-900/10 bg-slate-100/80 p-4 dark:border-white/5 dark:bg-slate-800/30"
					>
						{#if editingUpdateId === update.id}
							...
							<textarea
								...
								rows="3"
								class="block w-full rounded-md border-0 bg-slate-100 py-3 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-900/10 placeholder:text-gray-500 focus:ring-2 focus:ring-inset focus:ring-indigo-500 dark:bg-slate-950/50 dark:text-white dark:ring-white/10 sm:text-sm sm:leading-6"
							></textarea>
						...
						{:else}
							<!-- View Mode -->
							<p class="whitespace-pre-wrap text-slate-700 dark:text-slate-200">{update.content}</p>
								...
								{#if $user && update.authorId === $user.uid}
									<div class="flex gap-1">
										<button
											...
											class="rounded-md px-3 py-2 text-xs text-blue-400 transition-colors hover:bg-blue-400/10 hover:text-blue-300"
										>
											Edit
										</button>
										<button
											onclick={() => handleDeleteUpdate(update.id)}
											class="rounded-md px-3 py-2 text-xs text-rose-400 transition-colors hover:bg-rose-400/10 hover:text-rose-300"
										>
											Delete
										</button>
									</div>
	...
	<SharePrayerModal {prayerId} sharedWith={prayer.sharedWith} bind:isOpen={showShareModal} />
	{#if isOwner}
		<AddUpdateModal bind:isOpen={showAddUpdateModal} {prayerId} />
	{/if}
{/if}
